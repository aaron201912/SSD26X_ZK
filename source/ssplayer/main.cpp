...
int main(int argc, char *argv[]) 
{
    ...
    mm_player_set_opts("resolution", "8294400", 0);		// 3840*2160
    ...
    while(!bExit)
    {
        ...
        if(i_client.Read(recvevt) > 0)
        {
            ...
            switch(recvevt.EventType)
            {
                case IPC_COMMAND_OPEN:
                {
					...
					mm_player_set_opts("audio_device", "", recvevt.stPlData.aodev);
					mm_player_set_opts("video_only", "", recvevt.stPlData.video_only);
					mm_player_set_opts("audio_only", "", recvevt.stPlData.audio_only);
					mm_player_set_opts("video_ratio", "", AV_SCREEN_MODE);
					mm_player_set_opts("video_rotate", "", recvevt.stPlData.rotate);	// AV_ROTATE_NONE
					mm_player_set_opts("enable_scaler", "", 1);
					mm_player_set_opts("resolution", "8294400", 0);		// 3840*2160
					
					
                    ret = mm_player_open(recvevt.stPlData.filePath, recvevt.stPlData.x, recvevt.stPlData.y, recvevt.stPlData.width, recvevt.stPlData.height);
                    if (ret < 0) {
                        memset(&sendevt,0,sizeof(IPCEvent));
                ...
                {
                    mm_player_seek2time(recvevt.stPlData.misc);
                }
                break;
                ...
                case IPC_COMMAND_SET_VOLUMN:
                    mm_player_set_volumn(recvevt.stPlData.volumn);
                break;

                case IPC_COMMAND_SET_MUTE:
                    mm_player_set_mute(recvevt.stPlData.mute);
                break;
        ...
        if (time_now.tv_sec - time_last.tv_sec >= 1 && g_playing) {
            ...
            if(o_server.Init()) {
                double position;
                int ret = mm_player_getposition(&position);
                if (ret >= 0) {
                    memset(&sendevt,0,sizeof(IPCEvent));
                    sendevt.EventType = IPC_COMMAND_GET_POSITION;
                    sendevt.stPlData.misc = position;
        ...
        // 异常处理或者播放完成
        if (g_mmplayer && g_playing) {
            if (g_mmplayer->play_status > 0) {
                if(!o_server.Init()) {
                    printf("Main Process Not start!!!\n");
                    g_ipc_error = true;
                } else {
                    ...
                    sendevt.stPlData.status = (g_mmplayer->audio_complete | g_mmplayer->video_complete);
                    o_server.Send(sendevt);
                    g_mmplayer->play_status = 0;
                    av_log(NULL, AV_LOG_INFO, "my_player has played complete!\n");
                }
            } else if (g_mmplayer->play_status < 0) {
                if(!o_server.Init()) {
                    printf("Main Process Not start!!!\n");
                    g_ipc_error = true;
                } else {
                    memset(&sendevt,0,sizeof(IPCEvent));
                    sendevt.EventType = IPC_COMMAND_ERROR;
                    sendevt.stPlData.status = g_mmplayer->play_status;
                    o_server.Send(sendevt);
