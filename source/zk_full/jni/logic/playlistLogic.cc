...
int CreateFileTree(FileTree_t *root)
{
    ...
    snprintf(root->time, sizeof(root->time), "%d/%d/%d %d:%d:%d %s", modifytm->tm_year+1900, modifytm->tm_mon+1,
    ...
    while ((ent=readdir(pDir)) != NULL)
    {
        ...
        if (!strcmp(root->name, "/"))
            snprintf(child->name, 256, "%s%s", root->name, ent->d_name);
        else
            snprintf(child->name, 256, "%s/%s", root->name, ent->d_name);
        ...
        else
        {
            ...
            snprintf(child->time, sizeof(child->time), "%d/%d/%d %d:%d:%d %s", modifytm->tm_year+1900, modifytm->tm_mon+1,
                    getDateOfMonth(modifytm->tm_year+1900, modifytm->tm_mon+1, modifytm->tm_yday+1),
                    modifytm->tm_hour, modifytm->tm_min, modifytm->tm_sec,
                    getDayOfWeek(modifytm->tm_wday));
        }
...
void GetPrevFile(char *pCurFileFullName, char *pPrevFileFullName, int prevFilePathLen)
{
	...
	for (i = 0; i < (int)(sFileChildren.size()); i++)
	{
		...
		snprintf(tmpPath, sizeof(tmpPath), "%s/%s", g_curDir, childInfo.name);
...
//	strcpy(tmp, g_selectPath);
	...
	memset(g_selectPath, 0, sizeof(g_selectPath));
	snprintf(g_selectPath, sizeof(g_selectPath), "%s/%s", g_curDir, childInfo.name);
