...
int SSTAR_AI_StartRecord()
{
	...
	for (i = 0; i < stAiSetAttr.u32ChnCnt; i++)
    {
        ...
        MI_SYS_SetChnOutputPortDepth(0, &stAiChn0OutputPort0, 4, 8);
...
int SSTAR_AO_SetVolume(MI_AUDIO_DEV aoDevId, int vol)
{
	MI_AO_SetVolume(aoDevId, 0, vol - 60, E_MI_AO_GAIN_FADING_OFF);
	return 0;
}

int SSTAR_AO_StartPlayFile(MI_AUDIO_DEV aoDevId, char *pPcmFile, int volume)
{
	MI_S32 s32Ret = MI_SUCCESS;
...
int SSTAR_AO_StopPlayFile(MI_AUDIO_DEV aoDevId)
{
